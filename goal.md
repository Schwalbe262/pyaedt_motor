...
- Current production replay evidence is available: the ten running tasks have produced 175 fetched rows so far; filtering de-duplicates retry artifacts, keeps 144 unique new `mesh_time_fine` ok rows, and passes the `partial175_bomfix` quality gate with zero physical sanity violations.
- LightGBM retraining on the combined `partial175_bomfix` dataset now preserves all case IDs and has 0 invalid training rows, but still misses the R2 target with min R2 0.703131808625 and avg R2 0.817569261833.
