...
- Latest scheduler sample: batch2 has completed=224, running=3, cancelled=3; batch3 has queued=64, running=127, completed=9; batch4 has queued=6, so active queued/running FEA is at the clarified cap of 200.
- Batch3 partial009 is ok=5/failed=4, with failed cases029/043/050/109 all `analysis_returned_false=True`; current failure-pattern rules are diagnostic only because the prior candidates now also match at least one ok row or miss failed case109.
- Batch4 `mesh_time_fine` plan has 200 unique new source cases, excludes batch1/batch2/batch3 source IDs plus the conservative rule `magnet_height_ratio>=0.942,magnet_setback_ratio>=0.121`, and cases001-006 were submitted as `/tasks` 9130-9133 and 9135-9136.
