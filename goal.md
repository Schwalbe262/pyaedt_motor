...
- Latest scheduler sample: batch3 has completed=198/running=2, batch4 has completed=47/failed=15/cancelled=94/queued=41/running=112, batch5 cases001-044 are queued, and total queued/running/attaching FEA is at the clarified cap of 200.
- Batch3 partial198 is ok=189/failed=9, with failed cases029/043/050/109/134/139/147/158/169 still diagnostic; batch4 partial047 is ok=41/failed=6, with failed cases028/075/108/118/132/136 missing required transient outputs; batch2p199+batch3p198+batch4p047 keeps 13,807 quality-passing rows with zero duplicate case IDs, zero missing required rows, and zero physical-sanity violations after filtering.
...
- Batch5 `mesh_time_fine` has 200 unique new source cases, excludes all 800 batch1-4 source IDs plus the conservative rule `magnet_height_ratio>=0.942,magnet_setback_ratio>=0.121`, and has 0 overlap with prior batches; cases001-044 are submitted.
...
- Latest LightGBM retraining on `batch2p199_batch3p193_batch4p042` keeps 13,797 rows before model outlier removal and still misses the target with min R2 0.725130948332 and avg R2 0.826322998770; the best recent retrain remains partial174 with min R2 0.727178725767 and avg R2 0.828310891005, and `batch2p199_batch3p198_batch4p047` quality passed without retraining.
