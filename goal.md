...
- Current production replay evidence is available: the ten running tasks have produced 209 fetched rows including retry duplicates; filtering de-duplicates retry artifacts, keeps 172 unique new `mesh_time_fine` ok rows, and passes the `partial209_bomfix` quality gate with zero physical sanity violations.
- LightGBM retraining on the combined `partial209_bomfix` dataset now preserves all case IDs and has 0 invalid training rows, but still misses the R2 target with min R2 0.713489359979 and avg R2 0.814793669811.
