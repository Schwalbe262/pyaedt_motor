...
- Latest `slurm_scheduler` policy at main `06a0786` still prefers `/tasks` for existing remote directories, `/tasks/git` for Git-backed work, and `/jobs dynamic_packed_srun` for many-case packed simulation batches; `/jobs python_git` is compatibility-only and becomes an attached task.
...
- Current production replay evidence is available: the ten running tasks have produced 150 fetched rows so far; filtering de-duplicates retry artifacts, keeps 123 unique new `mesh_time_fine` ok rows, and passes the `partial150_bomfix` quality gate with zero physical sanity violations.
- LightGBM retraining on the combined `partial150_bomfix` dataset now preserves all case IDs and has 0 invalid training rows, but still misses the R2 target with min R2 0.696982666775 and avg R2 0.818653447281.
