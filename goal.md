...
- Latest scheduler sample: batch2 has completed=226, running=1, cancelled=3; batch3 has running=81, completed=119; batch4 has queued=85, running=33, so active queued/running FEA is at the clarified cap of 200.
- Batch3 partial107 is ok=98/failed=9, with failed cases029/043/050/109/134/139/147/158/169 all `analysis_returned_false=True`; candidate rules remain diagnostic because the narrow height/setback/shield rule matches 5 ok rows, and the broader height/setback rule matches 26 ok rows.
- Batch4 `mesh_time_fine` plan has 200 unique new source cases, excludes batch1/batch2/batch3 source IDs plus the conservative rule `magnet_height_ratio>=0.942,magnet_setback_ratio>=0.121`, and cases001-118 are queued/running as `/tasks` through 9293.
- LightGBM retraining on `partial219_bomfix` plus batch2 partial199 and batch3 partial107 keeps 13,675 rows with no invalid training rows, but still misses the target with min R2 0.709314380095 and avg R2 0.822486596958.
