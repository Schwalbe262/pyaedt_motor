...
- Direct, subprocess, Slurm shell, and controller entrypoints now enforce explicit case-plan guards; 50 is the active queued/running concurrency cap, not the total project solve cap, so larger case plans may continue through bounded refill waves.
- Subprocess splitting now rejects duplicate explicit `case_id`s before worker CSV generation.
- Scheduler endpoint is verified at `http://localhost:8000`; dry-run-first scheduler preparation now supports the updated `/api/tasks` JSON API for service clients plus legacy `/tasks` form posts for existing remote working trees with exact `account_name`, `env_profile`, and optional small case-CSV bootstrap.
- Latest `slurm_scheduler` policy at main `1c493ad8` keeps `/tasks` with `fea_bursty` valid for existing remote FEA, adds `/api/tasks` JSON for service-to-service clients with `dedupe_key` and `max_workers_per_node`, uses `/tasks/git` or `/api/tasks/git` for Git-backed work, `/api/task-capacity` for capacity checks, and `/jobs dynamic_packed_srun` for packed simulation batches; `/jobs python_git` is compatibility-only.
...
- Retry1 of the 20 failed first-batch geometries used `/tasks/git` tasks 8358-8361 and completed 20/20 with repeated AEDT `analysis=False`; next waves should diagnose or avoid that geometry set while honoring the current <=50 FEA concurrency policy.
...
- Historical scheduler sample: batch3 completed=200, batch4 reached completed=98/failed=15/cancelled=94/running=102, and batch5 cases001-100 were submitted under the former cap of 200; the current project cap is 50.
- Batch3 partial200 is ok=191/failed=9, with failed cases029/043/050/109/134/139/147/158/169 still diagnostic; batch4 partial098 is ok=87/failed=11, with failed cases028/075/108/118/132/136/169/174/176/181/185 missing required transient outputs; batch5 partial003 is failed=3/ok=0 with the same missing-transient-output class; batch2p199+batch3p200+batch4p098 keeps 13,855 quality-passing rows with zero duplicate case IDs, zero missing required rows, and zero physical-sanity violations after filtering.
- Batch4 cases081-095 failed from submission plumbing (`remote/cases.csv` missing), not simulation quality; bad nonterminal cases096-189 were cancelled, corrected cases081-200 were submitted through `/api/tasks` with deterministic `dedupe_key`s and per-case bootstrapped remote CSV paths.
- Batch5 `mesh_time_fine` has 200 unique new source cases, excludes all 800 batch1-4 source IDs plus the conservative rule `magnet_height_ratio>=0.942,magnet_setback_ratio>=0.121`, and has 0 overlap with prior batches; cases001-100 are submitted.
- `sync_ipmsm_scheduler_replay.py` now automates read-only scheduler sampling, missing completed-result fetch, partial selected CSV generation, exact-slot refill manifests/submissions, explicit case-number refill, per-case remote CSV bootstrap, and active-cap accounting across all `ipmsm-batch%-fea-%` tasks.
- Latest LightGBM retraining on `batch2p199_batch3p200_batch4p077` keeps 10,341 rows after model output-outlier removal and still misses the target with min R2 0.723311248898 and avg R2 0.821010077138; keeping output outliers on the earlier p047 dataset was much worse with min R2 0.307913855989 and avg R2 0.689162366840, so output-outlier removal remains justified.
- Replay/source drift analysis on the p077 training-ready dataset matched 630/630 replay rows to source rows and showed high target drift, especially torque, solid loss, and efficiency; a replay-replaces-source dataset passed quality gates but retraining worsened to min R2 0.709893143232 and avg R2 0.818927270778, so simple source-row replacement is not the fix.
- Replay-only training on batch2p199+batch3p200+batch4p092 has only 465 ok rows and 320 valid post-outlier rows; it passes only Lq and fails 7/8 targets with min R2 0.543859475103 and avg R2 0.718521259918, so high-quality replay-only evidence is still data-limited.
- Output IQR outlier analysis on p092 matches the training filter counts: combined rows_with_any_output_outlier=3,494/13,849 and replay-only rows_with_any_output_outlier=145/465; efficiency and torque targets dominate removals in both datasets.
...
- Next focus is polling batch3/batch4/batch5 tasks, fetching completed result CSV summaries only, updating partial gates, and retraining only after enough fetched rows justify it.
...
- Use `/api/tasks` JSON with deterministic `dedupe_key` and active-cap accounting across all nonterminal IPMSM FEA tasks for automated remote-cwd setup/analyze submissions, keep legacy `/tasks` form posts only for manual compatibility, use `/tasks/git` or `/api/tasks/git` for Git-backed work, and use `dynamic_packed_srun` with one explicit case row per scheduler `SIMULATION_ID` only for packed replay batches that fit current scheduler policy.
