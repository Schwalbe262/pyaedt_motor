...
- Latest scheduler sample: batch3 has completed=193/running=7, batch5 cases001-028 are queued, and total queued/running/attaching FEA is at the clarified cap of 200.
- Batch3 partial193 is ok=184/failed=9, with failed cases029/043/050/109/134/139/147/158/169 still diagnostic; batch2p199+batch3p193 keeps 13,761 quality-passing rows with zero duplicate case IDs, zero missing required rows, and zero physical-sanity violations after filtering.
...
- Batch5 `mesh_time_fine` has 200 unique new source cases, excludes all 800 batch1-4 source IDs plus the conservative rule `magnet_height_ratio>=0.942,magnet_setback_ratio>=0.121`, and has 0 overlap with prior batches; cases001-028 are submitted.
...
- Latest LightGBM retraining on `partial219_bomfix` plus batch2 partial199 and batch3 partial181 keeps 13,749 rows with no invalid training rows, but still misses the target with min R2 0.700211567668 and avg R2 0.813982163291; the best recent retrain remains partial174 with min R2 0.727178725767 and avg R2 0.828310891005, and partial184 quality passed without retraining.
