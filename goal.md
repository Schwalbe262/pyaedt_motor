...
- Current production replay evidence is available: the ten running tasks have produced 110 fetched rows so far; filtering de-duplicates retry artifacts, keeps 93 unique new `mesh_time_fine` ok rows, and passes the `partial110_bomfix` quality gate with zero physical sanity violations.
- LightGBM retraining on the combined `partial110_bomfix` dataset now preserves all case IDs and has 0 invalid training rows, but still misses the R2 target with min R2 0.71409574595 and avg R2 0.819444082622.
