...
- The replay selector can exclude previous source IDs and numeric high-risk rules; batch2 `mesh_time_fine` has all 200 cases submitted through `/tasks`, with explicit partial197 evidence at result_rows=197, ok=191, failed=6, combined_kept=13,575, duplicate source cases137/138 excluded from gate math, and zero physical-sanity violations.
...
- Latest scheduler sample: batch2 has completed=224, running=3, cancelled=3; batch3 has queued=48, running=142, completed=10; batch4 has queued=7, so active queued/running FEA is at the clarified cap of 200.
- Batch3 partial010 is ok=5/failed=5, with failed cases029/043/050/109/134 all `analysis_returned_false=True`; candidate rule `magnet_height_ratio>=0.926,magnet_setback_ratio>=0.118,magnet_shield_thick<=2.514` matches 5/5 failed and 0/5 ok in partial010 but is not confirmed because it also matches many historical ok rows.
- Batch4 `mesh_time_fine` plan has 200 unique new source cases, excludes batch1/batch2/batch3 source IDs plus the conservative rule `magnet_height_ratio>=0.942,magnet_setback_ratio>=0.121`, and cases001-007 were submitted as `/tasks` 9130-9133, 9135-9136, and 9139.
