...
- Latest scheduler sample: batch2 has completed=226, running=1, cancelled=3; batch3 has running=119, completed=81; batch4 has queued=65, running=15, so active queued/running FEA is at the clarified cap of 200.
- Batch3 partial081 is ok=72/failed=9, with failed cases029/043/050/109/134/139/147/158/169 all `analysis_returned_false=True`; current candidate rules remain diagnostic because the narrow height/setback/shield rule matches 2 ok rows, and broader rules match 12 ok rows.
- Batch4 `mesh_time_fine` plan has 200 unique new source cases, excludes batch1/batch2/batch3 source IDs plus the conservative rule `magnet_height_ratio>=0.942,magnet_setback_ratio>=0.121`, and cases001-080 are queued/running as `/tasks` through 9247.
- LightGBM retraining on `partial219_bomfix` plus batch2 partial199 and batch3 partial072 keeps 13,640 rows with no invalid training rows, but still misses the target with min R2 0.706808077388 and avg R2 0.818738312449.
