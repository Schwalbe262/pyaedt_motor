...
- Current production replay evidence is available: the ten running tasks have produced 207 fetched rows including retry duplicates; filtering de-duplicates retry artifacts, keeps 170 unique new `mesh_time_fine` ok rows, and passes the `partial207_bomfix` quality gate with zero physical sanity violations.
- LightGBM retraining on the combined `partial207_bomfix` dataset now preserves all case IDs and has 0 invalid training rows, but still misses the R2 target with min R2 0.703540619549 and avg R2 0.816294706022.
