...
- Current production replay evidence is available: the ten running tasks have produced 97 fetched rows so far; filtering de-duplicates retry artifacts, keeps 81 unique new `mesh_time_fine` ok rows, and passes the `partial97_bomfix` quality gate with zero physical sanity violations.
- LightGBM retraining on the combined `partial97_bomfix` dataset now preserves all case IDs and has 0 invalid training rows, but still misses the R2 target with min R2 0.700955899961 and avg R2 0.814627171852.
