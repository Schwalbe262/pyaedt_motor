...
- Latest scheduler sample: batch2 has completed=226, running=1, cancelled=3; batch3 has running=47, completed=153; batch4 has completed=1, queued=70, running=82, so active queued/running FEA is at the clarified cap of 200.
- Batch3 partial153 is ok=144/failed=9, with failed cases029/043/050/109/134/139/147/158/169 all `analysis_returned_false=True`; candidate rules remain diagnostic because the narrow height/setback/shield rule matches 5 ok rows, and the broader height/setback rule matches 26 ok rows.
- Batch4 `mesh_time_fine` plan has 200 unique new source cases, excludes batch1/batch2/batch3 source IDs plus the conservative rule `magnet_height_ratio>=0.942,magnet_setback_ratio>=0.121`, and cases001-153 are submitted as `/tasks` through 9332; first completed case028 failed `analysis_returned_false=True`.
- `sync_ipmsm_scheduler_replay.py` now automates read-only scheduler sampling, missing completed-result fetch, partial selected CSV generation, and exact-slot batch4 refill manifests/submissions; targeted sync/summarizer/submit tests passed 17/17.
- LightGBM retraining on `partial219_bomfix` plus batch2 partial199 and batch3 partial138 keeps 13,706 rows with no invalid training rows, but still misses the target with min R2 0.72288624746 and avg R2 0.822089378838.
