...
- The replay selector can exclude previous source IDs and numeric high-risk rules; batch2 `mesh_time_fine` has all 200 cases submitted through `/tasks`, with explicit partial190 evidence at result_rows=190, ok=184, failed=6, duplicate source cases137/138 excluded from gate math, and zero physical-sanity violations.
...
- Latest scheduler sample: batch2 has completed=217, running=10, cancelled=3; batch3 has queued=127, attaching=1, running=70, completed=3; batch3 cases029/043/050 all failed with `analysis_returned_false=True`.
