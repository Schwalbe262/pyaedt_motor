...
- Latest scheduler sample: batch2 still has one running task; batch3 has completed=169/running=31; batch4 has completed=6/failed=11/queued=90/running=78, so active queued/running/attaching FEA is at the clarified cap of 200.
- Batch3 partial169 is ok=160/failed=9, with failed cases029/043/050/109/134/139/147/158/169 still diagnostic; batch2p199+batch3p169 keeps 13,737 quality-passing rows with zero duplicate case IDs, zero missing required rows, and zero physical-sanity violations after filtering.
- Batch4 `mesh_time_fine` plan has 200 unique new source cases, excludes batch1/batch2/batch3 source IDs plus the conservative rule `magnet_height_ratio>=0.942,magnet_setback_ratio>=0.121`, and cases001-185 are submitted; cases163-185 used the updated `/api/tasks` JSON endpoint with deterministic `dedupe_key`s.
- `sync_ipmsm_scheduler_replay.py` now automates read-only scheduler sampling, missing completed-result fetch, partial selected CSV generation, and exact-slot batch4 refill manifests/submissions; targeted sync/submit tests passed after updating submissions to scheduler `/api/tasks`.
- Latest LightGBM retraining remains `partial219_bomfix` plus batch2 partial199 and batch3 partial163, with 13,731 rows and no invalid training rows, but still misses the target with min R2 0.715060465762 and avg R2 0.822021723141; partial169 quality passed but was not retrained yet.
