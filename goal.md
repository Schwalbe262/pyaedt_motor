...
- Direct, subprocess, Slurm shell, and controller entrypoints enforce explicit case-plan guards; the user-approved active queued/running concurrency cap is 300, so one adaptive 300-case plan may run as a single wave.
...
- Retry1 of the 20 failed first-batch geometries used `/tasks/git` tasks 8358-8361 and completed 20/20 with repeated AEDT `analysis=False`; next waves should diagnose or avoid that geometry set while honoring the current <=300 FEA concurrency policy.
...
- Historical scheduler sample: batch3 completed=200, batch4 reached completed=98/failed=15/cancelled=94/running=102, and batch5 cases001-100 were submitted under the former cap of 200; the current user-approved project cap is 300.
