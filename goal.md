...
- The replay selector can exclude previous source IDs and numeric high-risk rules; batch2 `mesh_time_fine` has all 200 cases submitted through `/tasks`, with explicit partial192 evidence at result_rows=192, ok=186, failed=6, duplicate source cases137/138 excluded from gate math, and zero physical-sanity violations.
...
- Latest scheduler sample: batch2 has completed=219, running=8, cancelled=3; batch3 has queued=112, running=85, completed=3; batch3 cases029/043/050 all failed with `analysis_returned_false=True`.
