...
- Latest scheduler sample: batch2 has completed=226, running=1, cancelled=3; batch3 has running=55, completed=145; batch4 has completed=1, queued=74, running=70, so active queued/running FEA is at the clarified cap of 200.
- Batch3 partial145 is ok=136/failed=9, with failed cases029/043/050/109/134/139/147/158/169 all `analysis_returned_false=True`; candidate rules remain diagnostic because the narrow height/setback/shield rule matches 5 ok rows, and the broader height/setback rule matches 26 ok rows.
- Batch4 `mesh_time_fine` plan has 200 unique new source cases, excludes batch1/batch2/batch3 source IDs plus the conservative rule `magnet_height_ratio>=0.942,magnet_setback_ratio>=0.121`, and cases001-145 are submitted as `/tasks` through 9326; first completed case028 failed `analysis_returned_false=True`.
