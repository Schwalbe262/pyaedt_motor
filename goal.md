...
- Latest scheduler sample: batch2 still has one running task; batch3 has completed=176/running=24; batch4 has completed=9/failed=15/cancelled=94/queued=96/running=79, so active queued/running/attaching FEA is back at the clarified cap of 200.
- Batch3 partial176 is ok=167/failed=9, with failed cases029/043/050/109/134/139/147/158/169 still diagnostic; batch2p199+batch3p176 keeps 13,744 quality-passing rows with zero duplicate case IDs, zero missing required rows, and zero physical-sanity violations after filtering.
- Batch4 cases081-095 failed from submission plumbing (`remote/cases.csv` missing), not simulation quality; bad nonterminal cases096-189 were cancelled, and corrected cases081-184 were resubmitted through `/api/tasks` with deterministic `dedupe_key`s and per-case bootstrapped remote CSV paths.
...
- Latest LightGBM retraining on `partial219_bomfix` plus batch2 partial199 and batch3 partial174 keeps 13,742 rows with no invalid training rows, but still misses the target with min R2 0.727178725767 and avg R2 0.828310891005; partial176 quality passed but was not retrained because it adds only two ok rows beyond partial174.
