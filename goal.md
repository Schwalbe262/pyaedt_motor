...
- Current production replay evidence is available: the ten running tasks have produced 137 fetched rows so far; filtering de-duplicates retry artifacts, keeps 113 unique new `mesh_time_fine` ok rows, and passes the `partial137_bomfix` quality gate with zero physical sanity violations.
- LightGBM retraining on the combined `partial137_bomfix` dataset now preserves all case IDs and has 0 invalid training rows, but still misses the R2 target with min R2 0.70819377136 and avg R2 0.820261489083.
