...
- The replay selector can exclude previous source IDs and numeric high-risk rules; batch2 `mesh_time_fine` has all 200 cases submitted through `/tasks`, with explicit partial194 evidence at result_rows=194, ok=188, failed=6, duplicate source cases137/138 excluded from gate math, and zero physical-sanity violations.
...
- Latest scheduler sample: batch2 has completed=221, running=6, cancelled=3; batch3 has queued=104, running=93, completed=3; batch3 cases029/043/050 all failed with `analysis_returned_false=True`.
