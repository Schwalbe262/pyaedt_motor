...
- Latest scheduler sample: batch2 has completed=226, running=1, cancelled=3; batch3 has running=93, completed=107; batch4 has queued=79, running=27, so active queued/running FEA is at the clarified cap of 200.
...
- Batch4 `mesh_time_fine` plan has 200 unique new source cases, excludes batch1/batch2/batch3 source IDs plus the conservative rule `magnet_height_ratio>=0.942,magnet_setback_ratio>=0.121`, and cases001-106 are queued/running as `/tasks` through 9280.
