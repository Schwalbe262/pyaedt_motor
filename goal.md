...
- Latest scheduler sample: batch2 still has one running task; batch3 has completed=183/running=17; batch4 has completed=11/failed=15/cancelled=94/queued=89/running=93, so active queued/running/attaching FEA is at the clarified cap of 200.
- Batch3 partial183 is ok=174/failed=9, with failed cases029/043/050/109/134/139/147/158/169 still diagnostic; batch2p199+batch3p183 keeps 13,751 quality-passing rows with zero duplicate case IDs, zero missing required rows, and zero physical-sanity violations after filtering.
- Batch4 cases081-095 failed from submission plumbing (`remote/cases.csv` missing), not simulation quality; bad nonterminal cases096-189 were cancelled, corrected cases081-189 were resubmitted, and cases190-193 were submitted through `/api/tasks` with deterministic `dedupe_key`s and per-case bootstrapped remote CSV paths.
- `sync_ipmsm_scheduler_replay.py` now automates read-only scheduler sampling, missing completed-result fetch, partial selected CSV generation, exact-slot batch4 refill manifests/submissions, explicit case-number refill, and per-case remote CSV bootstrap for shared remote-cwd concurrency.
- Latest LightGBM retraining on `partial219_bomfix` plus batch2 partial199 and batch3 partial181 keeps 13,749 rows with no invalid training rows, but still misses the target with min R2 0.700211567668 and avg R2 0.813982163291; the best recent retrain remains partial174 with min R2 0.727178725767 and avg R2 0.828310891005, and partial183 quality passed without retraining.
...
- Use `/api/tasks` JSON with deterministic `dedupe_key` for automated remote-cwd setup/analyze submissions, keep legacy `/tasks` form posts only for manual compatibility, use `/tasks/git` or `/api/tasks/git` for Git-backed work, and use `dynamic_packed_srun` with one explicit case row per scheduler `SIMULATION_ID` only for packed replay batches that fit current scheduler policy.
