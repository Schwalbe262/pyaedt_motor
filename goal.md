...
- Current production replay evidence is available: the ten running tasks have produced 194 fetched rows so far; filtering de-duplicates retry artifacts, keeps 160 unique new `mesh_time_fine` ok rows, and passes the `partial194_bomfix` quality gate with zero physical sanity violations.
- LightGBM retraining on the combined `partial194_bomfix` dataset now preserves all case IDs and has 0 invalid training rows, but still misses the R2 target with min R2 0.724112210031 and avg R2 0.819459032508.
