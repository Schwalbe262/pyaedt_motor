...
- Latest scheduler sample: batch3 has completed=198/running=2, batch4 has completed=65/failed=15/cancelled=94/queued=15/running=120, batch5 cases001-062 are queued, and total queued/running/attaching FEA is at the clarified cap of 200.
- Batch3 partial198 is ok=189/failed=9, with failed cases029/043/050/109/134/139/147/158/169 still diagnostic; batch4 partial065 is ok=57/failed=8, with failed cases028/075/108/118/132/136/169/174 missing required transient outputs; batch2p199+batch3p198+batch4p065 keeps 13,823 quality-passing rows with zero duplicate case IDs, zero missing required rows, and zero physical-sanity violations after filtering.
...
- Batch5 `mesh_time_fine` has 200 unique new source cases, excludes all 800 batch1-4 source IDs plus the conservative rule `magnet_height_ratio>=0.942,magnet_setback_ratio>=0.121`, and has 0 overlap with prior batches; cases001-062 are submitted.
...
- Latest LightGBM retraining on `batch2p199_batch3p198_batch4p047` keeps 10,322 rows after model output-outlier removal and still misses the target with min R2 0.712941986612 and avg R2 0.824314927334; keeping output outliers is much worse with min R2 0.307913855989 and avg R2 0.689162366840, so output-outlier removal remains justified; `batch2p199_batch3p198_batch4p065` quality passed without retraining.
