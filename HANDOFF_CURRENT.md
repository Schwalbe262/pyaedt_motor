...
- 2026-06-16: production replay snapshots now have 207 fetched rows including retry duplicates; raw partial has 187 `ok`, 20 failed, and 19 retry duplicates, while `partial207_bomfix` keeps 13,374 rows with no blank/duplicate case IDs and no physical-sanity violations.
- 2026-06-16: tasks 8154, 8155, 8156, and 8158 are completed, while the other replay tasks still report `running`; `train_ipmsm_lightgbm.py --data simul_log_smoke\training_ready_physical_plus_mtf200_partial207_bomfix.csv --disable-tuning` still misses target with min R2 0.703540619549, avg 0.816294706022.
