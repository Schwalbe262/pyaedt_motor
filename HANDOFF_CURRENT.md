...
- 2026-06-17: batch2 has explicit partial197 summary at result_rows=197, ok=191, failed=6, combined_kept=13,575, duplicates=0, physical_sanity_violations=0; batch3 partial011 is 5 ok / 6 failed.
...
- Batch2 all cases001-200 are submitted; latest counts are completed=224, running=3, cancelled=3. Batch3 all cases001-200 are submitted; latest counts are queued=48, running=141, completed=11. Batch4 cases001-008 are submitted and queued.
...
7. Diagnose the batch3 `analysis=False` cluster before promoting a new exclusion rule; partial011 has 5 ok / 6 failed, and current candidate rules either miss failed rows or match many historical ok rows.
...
- Partial batch2 evidence is now result_rows=197, ok=191, failed=6, duplicates=0; batch4 plan excludes 600 prior source IDs and 2,927 conservative-rule rows, has 200 unique sources, and cases001-008 are queued under the 200 active cap.
