...
- 2026-06-17: batch2 has explicit partial199 summary at result_rows=199, ok=193, failed=6, combined_kept=13,577, duplicates=0, physical_sanity_violations=0; batch3 partial042 is 33 ok / 9 failed.
...
- Batch2 all cases001-200 are submitted; latest counts are completed=226, running=1, cancelled=3. Batch3 all cases001-200 are submitted; latest counts are queued=18, running=140, completed=42. Batch4 cases001-041 are submitted and queued; total active FEA is 200.
...
7. Diagnose the batch3 `analysis=False` cluster before promoting a new exclusion rule; partial042 has 33 ok / 9 failed, and the narrow height/setback/shield rule now has two ok false positives.
...
- Partial batch2 evidence is result_rows=199, ok=193, failed=6, duplicates=0; batch3 partial042 is 33 ok / 9 failed; batch4 plan excludes 600 prior source IDs and 2,927 conservative-rule rows, has 200 unique sources, and cases001-041 are queued under the 200 active cap.
