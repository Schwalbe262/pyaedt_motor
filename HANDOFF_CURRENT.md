...
- 2026-06-16: production replay snapshots now have 202 fetched rows including retry duplicates; raw partial has 182 `ok`, 20 failed, and 19 retry duplicates, while `partial202_bomfix` keeps 13,369 rows with no blank/duplicate case IDs and no physical-sanity violations.
- 2026-06-16: tasks 8154 and 8156 are completed, while the other replay tasks still report `running`; `train_ipmsm_lightgbm.py --data simul_log_smoke\training_ready_physical_plus_mtf200_partial202_bomfix.csv --disable-tuning` still misses target with min R2 0.714858045327, avg 0.820054146857.
