...
- 2026-06-16: production replay snapshots now have 60 fetched rows; raw partial has 55 `ok`, 5 failed, and 5 retry duplicates, while `partial60_bomfix` keeps 13,254 rows with no blank/duplicate case IDs and no physical-sanity violations.
- 2026-06-16: `train_ipmsm_lightgbm.py --data simul_log_smoke\training_ready_physical_plus_mtf200_partial60_bomfix.csv --disable-tuning` passed row gates with 0 invalid rows and 0 duplicate drops; R2 remains below target with min 0.7148232912, avg 0.821373174101.
...
- Scheduler reaches AEDT; current blocker is waiting for the remaining production replay rows, then filtering and retraining; failed row indexes 63, 67, 106, 123, and 146 are retry candidates after the 200-run guardrail is reviewed.
