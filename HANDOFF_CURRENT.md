...
- 2026-06-16: `python -m unittest discover -s tests` ran 137 tests and passed; touched-file py_compile and `git diff --check` passed.
...
- 2026-06-16: latest `slurm_scheduler` main is `5215dbb`; normal remote work should use `/tasks`, Git work `/tasks/git`, and `/jobs` only for packed simulation batches or compatibility.
...
- First successful AEDT setup, solve, and fixed-geometry 4-profile comparison evidence exists; no multi-geometry quality replay or R2-improving retraining evidence exists yet.
...
3. Use `replay_quality_cases_200.csv` or smaller fixed-geometry chunks for profile comparisons; first fixed chunk suggests `time_fine` is close to `mesh_time_fine` at lower runtime, but one geometry is not enough.
4. Update/restart the local scheduler service to latest `5215dbb` before relying on `/tasks`; current live service likely still has the older `~/.../task.sh` expansion bug seen in task 33.
...
- Scheduler helpers dry-run setup/analyze jobs, write full review manifests, redact large bootstrap env setup from stdout, support updated `/tasks`, exact `account_name`, and `dynamic_packed_srun` policy fields.
