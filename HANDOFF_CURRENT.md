...
- 2026-06-16: production replay snapshots now have 205 fetched rows including retry duplicates; raw partial has 185 `ok`, 20 failed, and 19 retry duplicates, while `partial205_bomfix` keeps 13,372 rows with no blank/duplicate case IDs and no physical-sanity violations.
- 2026-06-16: tasks 8154, 8156, and 8158 are completed, while the other replay tasks still report `running`; `train_ipmsm_lightgbm.py --data simul_log_smoke\training_ready_physical_plus_mtf200_partial205_bomfix.csv --disable-tuning` still misses target with min R2 0.719524961251, avg 0.816394577961.
