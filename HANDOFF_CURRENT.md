...
- 2026-06-17: batch3 partial176 keeps 13,744 quality-passing rows; latest retrain is partial174 and still fails `R^2 >= 0.95` with 8/8 target failures, min R2 0.727178725767, avg R2 0.828310891005; batch4 cases081-095 failed from missing `remote/cases.csv`, bad active cases096-189 were cancelled, and corrected cases081-184 were resubmitted with per-case bootstrapped CSVs.
...
- Batch2 still has one running task. Batch3 all cases001-200 are submitted with completed=176/running=24. Batch4 has completed=9/failed=15/cancelled=94/queued=96/running=79 after cancelling bad no-bootstrap cases096-189 and resubmitting corrected cases081-184; total active FEA is 200.
...
4. Resubmit remaining cancelled batch4 cases185-189 before advancing to new case numbers; automated refills must use `/api/tasks`, deterministic `dedupe_key`, explicit Ansys module env setup, and per-case bootstrapped remote CSV paths.
...
- Partial batch2 evidence is result_rows=199, ok=193, failed=6, duplicates=0; batch3 partial176 is 167 ok / 9 failed and `batch2p199_batch3p176` quality passed with 13,744 rows; batch4 old cases081-095 are plumbing failures, corrected cases081-184 are resubmitted, and case028 failed `analysis_returned_false=True`.
