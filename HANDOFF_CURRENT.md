...
- Batch2 is running through attached `fea_bursty` tasks after packed jobs 62-71 were cancelled before Slurm submission; current blocker is waiting for n107 tasks 8716/8722-8723/8729-8733.
...
1. Poll remaining `/api/tasks/8716`,`8722`-`8723`,`8729`-`8733` with filtered fields; fetch only per-task module result CSV row/status summaries.
...
- Partial batch2 evidence is now result_rows=87, ok=83, failed=4, duplicates=0; n107 case 087 is `ok`, cases 088-095 are running, and failed 091-092 `n107_module22` submit attempts did not create tasks.
