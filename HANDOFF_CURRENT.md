...
- 2026-06-17: batch2 has explicit partial199 summary at result_rows=199, ok=193, failed=6, combined_kept=13,577, duplicates=0, physical_sanity_violations=0; batch3 partial020 is 12 ok / 8 failed.
...
- Batch2 all cases001-200 are submitted; latest counts are completed=226, running=1, cancelled=3. Batch3 all cases001-200 are submitted; latest counts are queued=30, running=150, completed=20. Batch4 cases001-019 are submitted and queued.
...
7. Diagnose the batch3 `analysis=False` cluster before promoting a new exclusion rule; partial020 has 12 ok / 8 failed, and current candidate rules either miss failed rows or match many historical ok rows.
...
- Partial batch2 evidence is now result_rows=199, ok=193, failed=6, duplicates=0; batch4 plan excludes 600 prior source IDs and 2,927 conservative-rule rows, has 200 unique sources, and cases001-019 are queued under the 200 active cap.
